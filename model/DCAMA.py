...
from model.base.transformer import MultiHeadedAttention, PositionalEncoding, CrossAttention, ScoreMap_embedding_concat
...
class DCAMA(nn.Module):
    ...
    def __init__(self, backbone, pretrained_path, use_original_imgsize):
        ...
        self.score_map_attention_blocks = nn.ModuleList()
        for inch in self.feat_channels[1:]:
            self.score_map_attention_blocks.append(CrossAttention(inch))

        """
        make moduel for embedding score map
        """
        self.resols = [48, 24, 12]
        self.scoremap_embedding_blocks = nn.ModuleList()
        for h in self.resols:
            self.scoremap_embedding_blocks.append(ScoreMap_embedding_concat(h))        

        self.cross_entropy_loss = nn.CrossEntropyLoss()

    """
    calculate score map
    GAP mode : compress query feature with global average pooling
    Transformer mode : calculate score map based on query key value
    """
    def calculate_score_map(self, query_feats, support_feats, support_mask, mode='Transformer'):
        score_maps = []
        for idx, (query_feat, support_feat) in enumerate(zip(query_feats, support_feats)):
            b, c, h = query_feat.size()[0:3]

            if h == 96:
                score_maps.append(torch.ones((h,h)).unsqueeze(0).unsqueeze(0).to('cuda:0'))
                continue

            fg_mask = F.interpolate(support_mask.unsqueeze(1).float(), support_feat.size()[2:], 
                                 mode='bilinear', align_corners=True)
            bg_mask = 1-fg_mask
            bg_mask_flat = bg_mask.view(b,1,-1)
            
            support_fg_feat = torch.mul(support_feat,fg_mask)
            support_bg_feat = torch.mul(support_feat,bg_mask)

            # if mode == 'GAP':
            #     query_feat_gap = F.adaptive_avg_pool2d(query_feat, (1,1)).squeeze(-1)
            #     """
            #     [feature channel] 
            #     query_feat_gap :    b, c, 1
            #     spt_bg_feat_flat:   b, c, h*h
            #     """
            #     support_bg_feat_flat = support_bg_feat.view(b,c,-1)
            #     score_map = F.cosine_similarity(query_feat_gap, support_bg_feat_flat, dim=1).view(b,1,h,h)
            #     score_map = (score_map - torch.min(score_map))/(torch.max(score_map) - torch.min(score_map))
            #     score_maps.append(score_map)

            if mode == 'Transformer':
                # support_bg_feat_flat = support_bg_feat.view(b,c,-1)
                support_fg_feat_flat = support_fg_feat.view(b,c,-1)
                query_feat_flat = query_feat.view(b,c,-1)
                support_feat_flat = support_feat.view(b,c,-1)

                """
                convert spt feacture as query feature
                """
                if idx < self.stack_ids[1]:
                    converted_spt_bg_feat= self.score_map_attention_blocks[0](support_feat_flat,query_feat_flat,query_feat_flat)
                    emb_spt_bg_feat= self.score_map_attention_blocks[0].embedding_value(support_feat_flat)
                elif idx < self.stack_ids[2]:
                    converted_spt_bg_feat= self.score_map_attention_blocks[1](support_feat_flat,query_feat_flat,query_feat_flat)
                    emb_spt_bg_feat= self.score_map_attention_blocks[1].embedding_value(support_feat_flat)
                else:
                    converted_spt_bg_feat= self.score_map_attention_blocks[2](support_feat_flat,query_feat_flat,query_feat_flat)
                    emb_spt_bg_feat= self.score_map_attention_blocks[2].embedding_value(support_feat_flat)                
                sqtBg_qry_similarity = F.cosine_similarity(converted_spt_bg_feat, emb_spt_bg_feat, dim=1).view(b,1,-1)

                """
                convert spt feacture as support_fg feature
                """
                if idx < self.stack_ids[1]:
                    converted_spt_bg_feat= self.score_map_attention_blocks[0](support_feat_flat,support_fg_feat_flat,support_fg_feat_flat)
                    emb_spt_bg_feat= self.score_map_attention_blocks[0].embedding_value(support_feat_flat)
                elif idx < self.stack_ids[2]:
                    converted_spt_bg_feat= self.score_map_attention_blocks[1](support_feat_flat,support_fg_feat_flat,support_fg_feat_flat)
                    emb_spt_bg_feat= self.score_map_attention_blocks[1].embedding_value(support_feat_flat)
                else:
                    converted_spt_bg_feat= self.score_map_attention_blocks[2](support_feat_flat,support_fg_feat_flat,support_fg_feat_flat)
                    emb_spt_bg_feat= self.score_map_attention_blocks[2].embedding_value(support_feat_flat)                
                sqtBg_sptFg_similarity = F.cosine_similarity(converted_spt_bg_feat, emb_spt_bg_feat, dim=1).view(b,1,-1)

                score_map = torch.mul(sqtBg_qry_similarity - sqtBg_sptFg_similarity, bg_mask_flat)
                
                if h == 48:
                    score_map = self.scoremap_embedding_blocks[0](score_map)
                elif h == 24:
                    score_map = self.scoremap_embedding_blocks[1](score_map)
                elif h == 12:
                    score_map = self.scoremap_embedding_blocks[2](score_map)

                score_maps.append(score_map.view(b,1,h,h))
        return score_maps
        

    """
    suppress background feature of support feature vector
    """
    def suppress_bg_feats(self, query_feats, support_feats, support_mask):
        sup_support_feats = []
        score_maps = self.calculate_score_map(query_feats, support_feats, support_mask)

        for query_feat, support_feat, score_map in zip(query_feats, support_feats, score_maps):
            b, c, h = query_feat.size()[0:3]

            if h == 96:
                sup_support_feats.append(support_feat)
                continue

            mask = F.interpolate(support_mask.unsqueeze(1).float(), support_feat.size()[2:], 
                                 mode='bilinear', align_corners=True)
            support_fg_feat = torch.mul(support_feat,mask)
            support_bg_feat = torch.mul(support_feat,1-mask)
            sup_support_bg_feat= torch.mul( support_bg_feat, score_map )
            sup_support_feats.append(sup_support_bg_feat + support_fg_feat)
        
        return sup_support_feats
    ...
    def forward(self, query_img, support_img, support_mask):
        ...
        support_feats = self.suppress_bg_feats(query_feats, support_feats, support_mask.clone())

        logit_mask = self.model(query_feats, support_feats, support_mask.clone())

        return logit_mask
    
    def show_score_maps(self, batch, nshot, idx, mode = 'raw'):
        if mode == 'default':
            r""" show score maps """
            ...
            if nshot == 1:
                score_maps = self.calculate_score_map( self.extract_feats(query_img), self.extract_feats(support_imgs), support_masks.squeeze(0) )
                mean_score_map = []

                for i in range(len(mean_score_map)):
                    mean_score_map[i] = F.interpolate(mean_score_map[i].to('cpu'), (384,384), mode='bilinear',align_corners=True).squeeze(0).squeeze(0).to('cpu').detach()

                fig, axes = plt.subplots(4, 7, figsize=(14, 8))
                for i, ax in enumerate(axes.flatten()):
                    if i == 0:
                        ax.imshow(np.transpose(support_imgs.squeeze(0).to('cpu').numpy(), (1, 2, 0)))
                    elif i == 1:
                        ax.imshow(support_masks.squeeze(0).squeeze(0).to('cpu').numpy())
                    elif i == 2:
                        ax.imshow(np.transpose(query_img.squeeze(0).to('cpu').numpy(), (1, 2, 0)))
                    elif i == 3:
                        ax.imshow(torch.mean(torch.stack(score_maps), dim = 0), cmap='jet')
                    else:
                        ax.imshow(score_maps[i-4], cmap='jet')
                ...
                plt.savefig('./score_maps/score_map_{}.png'.format(idx), bbox_inches='tight', pad_inches=0)
        ...
        if mode == 'raw':
            ...
            if nshot == 1:
                ...
                for i in range(len(score_maps)):
                    score_maps[i] = F.interpolate(score_maps[i].to('cpu'), (384,384), mode='bilinear',align_corners=True).squeeze(0).squeeze(0).to('cpu').detach()
                ...
                for i, ax in enumerate(axes.flatten()):
                    if i == 0:
                        ...
                        ax.imshow(score_maps[i-4], cmap='jet', vmax = 1, vmin = 0)

                    # print("{}: mean={} max={} min={}".format(idx, torch.mean(score_maps[i-4]), torch.max(score_maps[i-4]), torch.min(score_maps[i-4])))
